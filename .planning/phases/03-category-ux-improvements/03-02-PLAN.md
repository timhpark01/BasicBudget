...
must_haves:
  truths:
    ...
    - "Toast notifications appear above modal content (proper z-index layering)"
  artifacts:
...
- autoFocus={true} draws attention to the input field immediately when modal opens, keyboard should appear automatically
...
npm start
```
   - Open Categories modal → Tap "+" to add → Should see placeholder "e.g., Groceries, Entertainment", keyboard appears immediately, cursor in field
   - Tap edit on existing category → Should see placeholder "Enter new name", keyboard appears immediately, and helper text "Renaming will update all expenses in this category"
3. Verify keyboard presentation happens automatically when modal opens (result of autoFocus={true})
  </verify>
  <done>
Category rename interface clearly distinguishes add mode (with examples) from edit mode (with impact warning), input auto-focuses on open with keyboard appearing automatically, user understands they're editing an existing category vs creating new
  </done>
</task>
...
Note: Toast component is configured at root level (app/_layout.tsx) and renders above all content including modals due to proper z-index layering from react-native-toast-message library. No additional configuration needed to ensure toasts appear above modal.
  </action>
  <verify>
...
5. **Verify toast z-index:** Toast should appear ABOVE the modal content, not behind it (proper layering from root-level Toast component)
  </verify>
  <done>
Success operations show non-blocking toasts at top of screen for 3 seconds (appearing above modal content), errors and destructive confirmations still use Alert.alert(), user can continue working immediately after successful operations (UX-04 fully addressed)
  </done>
</task>
...
2. Modify drag handle to be visually clearer (around line 171). CURRENT implementation shows "reorder-two" icon (line 175), enhance with darker color and instructional text:
...
**Rationale:**
- Change icon color from #999 to darker green #355e3b to make drag handle more prominent
- Add "Hold" text label to provide explicit instruction (addresses "easier to understand" in UX-03)
...
- Correct icon name is "reorder-two" (not "menu")
...
1. Read CategoriesModal.tsx and confirm:
   ...
   - Drag handle styling updated with darker green icon (#355e3b) and "Hold" text
   - Icon name is "reorder-two" (not "menu")
...
2. Test on physical device (haptics don't work in simulator):
   ...
   - Visual check: Drag handle shows green "reorder-two" icon + "Hold" text
   - Vibration timing: start of drag + completion (not during drag)
  </verify>
  <done>
Category drag mechanism is visually clearer (prominent green "reorder-two" icon + "Hold" text instruction), provides haptic feedback at start (medium impact) and completion (success notification), drag handle has accessibility labels for screen readers (UX-03 fully addressed)
  </done>
</task>
...
- Keyboard appears automatically when modal opens (autoFocus behavior verified)
- Category create/update/delete operations show toast notifications (not blocking alerts)
- Toasts appear ABOVE modal content (proper z-index layering verified)
- Errors and destructive confirmations still use Alert.alert() appropriately
- Drag handle has visual affordance (darker green "reorder-two" icon + "Hold" text)
...
- Plans 03-02 and 03-03 can run in parallel safely (no file conflicts, both depend only on 03-01 toast infrastructure)
</success_criteria>
